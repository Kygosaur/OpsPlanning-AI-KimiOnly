...
def validate_loopback_url(base_url: str) -> str:
    """Accept only explicit loopback IPs; hostnames are rejected to prevent DNS rebinding."""
    ...
    try:
        ...
        raise ValueError("Use an explicit loopback IP such as 127.0.0.1, not a hostname") from error
    if not address.is_loopback:
        raise ValueError("Remote LLM endpoints are disabled; only loopback addresses are allowed")
